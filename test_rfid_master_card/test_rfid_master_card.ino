...
byte master[5] = {0xE5,0xCD,0x09,0x53,0x72};  
...
void loop(){
  ...
  boolean rfidmipo_card = false; // my card
 ...
 if(data[0] < 16){
  ...
  for(int i=0; i<5; i++){
    //if(data[i] != rfidmipo[i]) rfidmipo_card = false; // if not my card, poses as fake. Here you can check the other cards allowed
    if(data[i] != master[i]) isMaster = false;
  }
  Serial.println();
  if (isMaster){ // A card found
    Serial.print(F("master card present.")); 
  }
  ...
  if (rfidmipo_card){// add other users with a logical or Welcome message and access permission
    ...
    doorLock.write(175); // door releases
    delay(5000); // waiting
    doorLock.write(5); // gate Latch
