...
public:Sweeper(){}  
  ...
  int readPos(){return servo.read();}

  void writePos(int pos){ servo.write(pos);}
};
...
int saveCard(int init_addr, byte data[], int n){
  ...
  for(byte y = 0; y < 5; y++){
     if(card.data[y] < 16){Serial.print("0");}
...
void loadcard(){
  ...
  for(byte i = 0; i < cnt; i++){
      ...
      Serial.println(aa);
      for(byte y = 0; y < 5; y++){
           if(cardlist[i].data[y] < 16){Serial.print("0");}
